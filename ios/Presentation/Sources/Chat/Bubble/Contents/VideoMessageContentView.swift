...
final class VideoMessageContentView: UIView, MessageContentView {
    ...
    func apply(
        ...
        guard case let .video(video) = model else { return }
        content = video
        mediaIcon.tintColor = chrome.iconTint
        bodyLabel.textColor = chrome.bodyColor
        bodyLabel.text = video.title
    }

    @objc private func tapped() {
        guard let content else { return }
        actions.onPreview?(.video(url: content.url, name: content.name, duration: content.duration))
    }
}
